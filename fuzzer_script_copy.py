...
class CorpusLoader:
        ...
        for root, _, files in os.walk(corpus_dir):
            for file in files:
                ...
                with open(path, "rb") as f:
                    data = f.read()
                    if data:
                        self.seeds.append({"data": data, "energy": 10.0})
        if not self.seeds:
    ...
    def get_random_seed(self):
        weights = [s["energy"] for s in self.seeds]
        return random.choices(self.seeds, weights=weights, k=1)[0]["data"]

    def add_seed(self, new_seed, energy, max_corpus_size):
        if not new_seed:
            return
        self.seeds.append({"data": new_seed, "energy": float(energy)})
...
def calculate_seed_energy(is_new_bug, hit_count, bug_type, reason, tier):
    if reason and reason.startswith("bug:"):
        if bug_type == "invalidity":
            return 5.0
        if is_new_bug:
            return 100.0
        if hit_count > 0 and (hit_count & (hit_count - 1)) == 0:
            return 50.0
        return 0.0
...
def main():
    ...
    try:
        ...
        warmup_seeds = [s["data"] for s in loader.seeds]
        ...
        for seed_input in warmup_seeds[:warmup_limit]:
            ...
            if is_interesting:
                ...
                if reason and reason.startswith("bug:"):
                    ...
                    dashboard.bug_hit_counts[dedup_fingerprint] = dashboard.bug_hit_counts.get(dedup_fingerprint, 0) + 1
        ...
        while True:
            ...
            if is_interesting:
                ...
                if reason and reason.startswith("bug:"):
                    ...
                    is_new_bug = dedup_fingerprint not in dashboard.unique_bug_fingerprints
                    dashboard.bug_hit_counts[dedup_fingerprint] = dashboard.bug_hit_counts.get(dedup_fingerprint, 0) + 1
                    hit_count = dashboard.bug_hit_counts[dedup_fingerprint]
                ...
                energy = calculate_seed_energy(is_new_bug, hit_count, bug_type, reason, tier)
                if energy > 0:
                    loader.add_seed(mutated_input, energy, args.max_corpus_size)
