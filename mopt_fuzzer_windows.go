...
func debugSigf(format string, args ...interface{}) {
	if !debugSignature {
		return
	}
	fmt.Printf("[DBG][SIG] "+format+"\n", args...)
}

func firstMeaningfulErrorLine(stdout, stderr string) string {
	combined := stdout + "\n" + stderr
	...
	for _, line := range lines {
		...
		if strings.Contains(line, "ParseException:") || strings.Contains(line, "Error:") || strings.Contains(line, "Exception:") || strings.Contains(line, "Bug:") {
			if len(line) > 140 {
				return line[:140]
			}
			return line
...
func (e *Evaluator) Evaluate(res *ExecutionResult, data []byte) (int, string, string) {
	...
	if res.IsError {
		...
		frames := extractTracebackFrames(res.Stdout, res.Stderr)
		errLine := firstMeaningfulErrorLine(res.Stdout, res.Stderr)
		
		e.CrashHitCounts[sig]++
		count := e.CrashHitCounts[sig]
		debugSigf("hit=%d sig=%s cat=%s exc=%s frames=%d err=%q", count, sig[:12], cat, exc, len(frames), errLine)
...
func executeTarget(ctx context.Context, config DriverConfig, inputData []byte, workerDir string, originalDir string) *ExecutionResult {
	...
	// Windows fallback: terminate the direct process on timeout/cancel.
    cleanup := func() {
        if cmd.Process != nil {
			_ = cmd.Process.Kill()
        }
...
func main() {
	...
	debugSigFlag := flag.Bool("debug-signature", false, "Enable debug logs for bug signature and tier-1 queue dedup decisions")
	flag.Parse()
	debugSignature = *debugSigFlag
			...
			if tier > 0 {
				...
				if tier == 1 {
					...
					if cat != "invalidity" && exc != "ParseException" {
						...
						if exists {
							debugSigf("queue=skip-duplicate sig=%s exc=%s", sig[:12], exc)
						}
						if !exists {
							seedQueue = append(seedQueue, &SeedEntry{
								Data: res.Mutated, Energy: HighEnergy, Tier: 1, Sig: sig,
							})
							debugSigf("queue=enqueued sig=%s exc=%s energy=%d", sig[:12], exc, HighEnergy)
						}
						queueMutex.Unlock()
					} else {
						debugSigf("queue=skipped-by-rule sig=%s cat=%s exc=%s", sig[:12], cat, exc)
					}
